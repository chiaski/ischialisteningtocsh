...
(function($){ 
        ...
        $.getJSON("https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&limit=10&user=" + user + "&api_key=" + apiKey + "&format=json",
            function(response){ //callback function, runs upon receiving response
                ...
                if(music.track[0].artist["#text"] == "Car Seat Headrest"){
                  ...
                  $(".container").css("background-image", "url(" + music.track[0].image[3]["#text"] + ")");
  ...
  $.getJSON("https://ws.audioscrobbler.com/2.0/?method=user.getweeklyartistchart&user=" + user + "&api_key=" + apiKey + "&format=json",
            function(response){ //callback function, runs upon receiving response
